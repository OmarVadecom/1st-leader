<div class="row">
    <div class="col-md-2">
        <div class="form-group">
            ...
            <select name="user_id" class="form-control selectproduct" required>
                ...
                <option value="{{ $user->id }}" @if(isset($edit)) {{ $user->id==$entry->user_id ? 'selected' : '' }}
                    @endif>
                    {{ $user->name }}
                </option>
                @endforeach
    ...
    <div class="col-md-2">
        <div class="form-group">
            ...
            {!! Form::time('time',date('H:i'),['class'=>'form-control','required'=>true]) !!}
        </div>
    </div>
    <div class="col-md-3">
        <div class="form-group">
...
<div class="row">
        ...
        <div class="col-md-12">
            <table id="added_products_table" class="table table-striped" style="text-align:center">
                ...
                <tbody id="table_body" class="productsadd">
                    @if(isset($edit))
                    @foreach($supplies as $k => $supply)
                    @if($supply->product()->first())
                    <tr
                        style="{{ isset( $product_id) && $supply->product_id == $product_id?'background-color: #5ec334  !important; ':'' }}">
                        <input type="hidden" name="supply_id[]" value="{{$supply->id}}">
                        <input type="hidden" name="product[]" value="{{$supply->product()->first()->id}}">
                        <input type="hidden" name="product_code_type[]"
                            value="{{$supply->product()->first()->code_type}}">
                        <td>{{ $supply->product()->first()->code }}</td>
                        <td>{{$supply->product()->first()->name }}</td>

                        <td><input value="{{$supply->quantity}}" type="number" data-number="{{$k}}" placeholder="الكميه"
                                min="0" class="quantities form-control productquantity quantity{{$k}}"
                                name="quantity[]">
                        </td>
                        <td><input value="{{number_format($supply->cost,2)}}" type="number" step="0.01"
                                data-number="{{$k}}" placeholder="التكلفة" min="0"
                                class="quantities form-control productcost cost{{$k}}" name="cost[]">
                        </td>
                        <td><input type="number" step="0.01" data-number="{{$k}}" placeholder="النسبه" min="0"
                                class="form-control addon_perc addon_perc{{$k}}" value="{{$supply->addon_perc}}"
                                name="addon_perc[]">
                            <input type="number" step="0.01" data-number="{{$k}}" placeholder="المبلغ" min="0"
                                class="form-control addon addon{{$k}}" name="addon[]" value="{{$supply->addon}}">
                        </td>
                        <td><input value="{{number_format($supply->price,2)}}" type="number" step="0.01"
                                data-number="{{$k}}" step="0.01" placeholder="السعر " min="0"
                                class="prices form-control productprice price{{$k}}" name="price[]"> </td>
                        <td>
                            <select name="stock_id[]" data-number="{{$k}}" class="select-stock" required>
                                <option value="">اختر المخزن</option>
                                @foreach($stocks as $stock)
                                <option value="{{$stock->id}}" {{$supply->stock_id==$stock->id? 'selected':'' }}>
                                    {{$stock->name}}</option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                            <select name="warehouse_id[]" data-number="{{$k}}" class="select-warehouse warehouse{{$k}}"
                                required>
                                <option value="">اختر المستودع</option>
                                @foreach($warehouses as $warehouse)
                                <option value="{{$warehouse->id}}" {{$supply->warehouse_id==$warehouse->id?
                                    'selected':'' }}>{{$warehouse->name}}
                                </option>
                                @endforeach
                            </select>
                        </td>
                        <td class="totals totalfir{{$k}}"></td>
    ...
    var k = '@if(isset($edit)){{count($supplies)}}@else{{0}}@endif';
