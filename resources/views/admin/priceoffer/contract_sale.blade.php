    ...
    <div class="container">
        <div class="row">
            ...
            <div class="content">
                ...
                <br>
                <label> طرف ثاني و عنوانه :</label> <span> {{$offer->customer->country}} </span>
                <label>&ensp; ص.ب : </label> <span> {{$offer->customer->street}} </span>
                <label >&ensp;بمدينة : </label> <span> {{$offer->customer->reg_city}}</span>
                <br>
                <label >&ensp;الرمز البريدي : </label><span> {{$offer->customer->reg_city}} </span>
                <label >&ensp;تليفون : </label><span> {{$offer->customer->resp_phone}}</span>
                <label >&ensp;رقم الهوية : </label><span> {{$offer->customer->dreb_phone}}</span>
                <label >&ensp;تاريخ : </label><span> {{$offer->date}}</span>
                <br>
                <label >&ensp;بكفالة السيد : </label><span> {{$offer->resp_name_sponsor	}}</span>
...
{{--                 {{$offer->products_id}}--}}
                ...
                <br>
                <label >&ensp;  فاتورة معمدة رقم</label> <span> {{$offer_number}} </span>
                <br>
                <label >&ensp; مسجلة يتاريخ : </label><span> {{$offer->date}}</span>
                <label >&ensp;  بمبلغ إجمالي : </label><span> {{$total_price}}</span>

                <br>
                <label > فقط على أن يدفع الطرف الثاني للطرف الاول 35 %من اصل القيمة كدفعة أولى مبلغ وقدره</label>
                <span> {{$percent_format}}</span>
                <br>
                <label >وقدره كتابة :</label><span> {{$word_percent}}</span> <!--there is concatenation here +    ريال سعودي لاغير  -->
                <br>
                <label > بموجب سند الوارد رقم </label><span>..........</span>
                <label >تاريخ</label><span>..........</span>
                <label > و الباقي بذمة الطرف الثاني للطرف الاول المبلغ المتبقي من العرض و قدره رقما</label><span> {{$percent_remainder}}</span>
                <br>
                <label > و قدره كتابة : </label><span> {{$word_remainder}}</span><!--there is concatenation here +    ريال الغير  -->
                <br>
        ...
        <div class="row">
            <div class="second-content">
                ...
                <span>{{$offer->resp_name_sponsor}}</span>
                <br>
