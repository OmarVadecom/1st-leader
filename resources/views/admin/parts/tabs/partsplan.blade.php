        ...
        <div class="col-md-12">
            @if(isset($charts) && $charts[0] != "")
                @foreach($charts as $key=>$chart)
                    <input id="filechart{{ $key }}_name" type="hidden" name="charts_names[]"
                        value="{{ $charts_names[$key] }}">
                            ...
                            <tbody>
                                @foreach($charts as $key=>$chart)
                                    <tr>
                                        ...
                                        <td> <a href="{{ url('/') }}/uploads/products-charts/{{ $chart }}"
                                                class="filechart{{ $key }}" download><input id="file{{ $key }}"
                                                    type="hidden" name="charts_edit[]" value="{{ $chart }}">
                                                {{ $chart }}</a></td>
                                        <td><i id="btu-filechart{{ $key }}" style="color:red"
